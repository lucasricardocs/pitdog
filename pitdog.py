...
def round_to_50_or_00(value):
    """Arredonda um valor para o múltiplo de 0.50 mais próximo."""
...
def local_search_optimization(item_prices, target_value, combination_size, max_iterations):
    ...
    approximating the target_value without exceeding it.
    """
    ...
    # Penalize solutions that exceed the target value
    ...
    for _ in range(max_iterations):
        ...
        neighbor[item_to_modify] = max(0.50, neighbor[item_to_modify])  # Minimum 0.50
...
# ----- Interface Streamlit -----
st.set_page_config(page_title="Análise de Vendas & Combinações", layout="wide")
...
e gera combinações de produtos que correspondam a esses totais, com as seguintes regras:
1. Valores sempre terminam em ,00 ou ,50
2. Combinação nunca ultrapassa o valor total
3. Se faltar valor, completa com cebolas (R$ 0,50)
""")
st.divider()
                ...
                for forma, total_pagamento in vendas_ordenadas.items():
                    ...
                    with st.spinner(f"Gerando combinação para {forma}..."):
                        ...
                        comb_bebidas_final, total_bebidas = adjust_with_onions(
                            comb_bebidas_rounded, bebidas_precos, target_bebidas
                        )
                    ...
                    with st.expander(f"**{forma}** (Total: {format_currency(total_pagamento)})", expanded=False):
                        ...
                        with col1:
                            ...
                            if comb_bebidas_final:
                                for nome, qtt in comb_bebidas_final.items():
