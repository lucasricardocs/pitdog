...
def generate_initial_combination(item_prices, combination_size):
    ...
    for name in chosen_names:
        combination[name] = random.uniform(1, 75)
    return combination
...
def local_search_optimization(item_prices, target_value, combination_size, max_iterations):
    ...
    approximating the target_value.
    """
...
if arquivo:
    with st.spinner(f'Processando "{arquivo.name}"... Por favor, aguarde.'):
        ...
        try:
            ...
            df_processed['Valor_Numeric'] = pd.to_numeric(df_processed['Valor'].str.replace('.', '', regex=False).str.replace(',', '.', regex=False), errors='coerce')
    ...
    st.image("https://cdn-icons-png.flaticon.com/128/3652/3652191.png", width=100)
