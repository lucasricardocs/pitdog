...
with tab2:
    ...
    if st.session_state.vendas_data is None:
        st.warning("""
        Nenhum dado de vendas encontrado. Por favor:
        1. Carregue um arquivo de transações na aba '📈 Resumo das Vendas'
...
with tab3:
    ...
    # Verificação inicial mais explícita
    if st.session_state.df_receipts.empty:
        st.warning("""
        Nenhum dado de recebimento encontrado. Você pode:
        1. Adicionar manualmente novos registros abaixo
        2. Carregar um arquivo de dados na aba '📈 Resumo das Vendas'
        """)
        
        if st.button("Carregar dados de exemplo (para teste)"):
            # Cria dados fictícios para demonstração
            example_data = {
                'Data': pd.date_range(end=datetime.today(), periods=30),
                'Dinheiro': [random.randint(100, 500) for _ in range(30)],
                'Cartao': [random.randint(200, 800) for _ in range(30)],
                'Pix': [random.randint(50, 300) for _ in range(30)]
            }
            st.session_state.df_receipts = pd.DataFrame(example_data)
